...
import { garchVolatility } from "@/lib/data";
    ...
    <div>
      ...
      <div className="card-elevated p-5 mb-6 bg-navy text-primary-foreground">
        ...
        <div className="text-xs mt-2 text-primary-foreground/70">
          ω = 0.024 · α₁ = 0.412 · β₁ = 0.498 · α₁+β₁ = 0.910
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KpiCard label="α₁ (ARCH effect)" value="0.412" tone="gold" hint={lang === "uz" ? "Qisqa muddatli shok" : "Short-term shock"} />
        <KpiCard label="β₁ (GARCH effect)" value="0.498" tone="info" hint={lang === "uz" ? "Uzoq xotira" : "Long memory"} />
        <KpiCard label="α₁ + β₁" value="0.910" tone="destructive" hint={lang === "uz" ? "< 1 barqaror ✓" : "< 1 stationary ✓"} />
        <KpiCard label={lang === "uz" ? "Yillik volatillik" : "Annual volatility"} value="14.8%" tone="navy" />
      </div>

      <SectionCard title={lang === "uz" ? "Shartli Volatillik Trendi" : "Conditional Volatility Trend"}
        subtitle="σₜ across 72 quarters">
        <ResponsiveContainer width="100%" height={340}>
          <AreaChart data={garchVolatility}>
            <defs>
